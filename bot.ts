...
const botUser = process.env.botUser || 'preview-bot';

const region = process.env.AWS_REGION;

const whitelistedUsers = process.env.whitelistedUsers ? process.env.whitelistedUsers.split(',') : ['allamand'];

const buildProject = process.env.buildProject || 'preview-bot';

const ecrRepository = process.env.ecrRepository || 'preview-bot-preview-images';
...
async function provisionPreviewStack(owner: string, repo: string, prNumber: number, requester: string) {
  try {
    console.debug('provisionPreviewStack[pr' + prNumber + ']');
    ...
    console.debug('provisionPreviewStack[pr' + prNumber + '] - startBuild');
    ...
    for (let i = 0; i < 150; i++) {
      console.debug('provisionPreviewStack[pr' + prNumber + '] - Wait for Build');
    ...
    console.debug('provisionPreviewStack[pr' + prNumber + '] - Build OK');
    ...
    console.debug('provisionPreviewStack[pr' + prNumber + '] - create or update CloudFormation stack');
    ...
    try {
      ...
      console.debug('provisionPreviewStack[pr' + prNumber + '] - stack ' + uniqueId + ' does not exist');
      if (err.message.endsWith('does not exist')) {
        stackExists = false;
      } else {
        console.debug('provisionPreviewStack[pr' + prNumber + '] - Unknown error: ' + err);
        throw err;
      }
    }
    ...
    if (stackExists) {
      ...
      console.debug('provisionPreviewStack[pr' + prNumber + '] - Create CloudFormation Stack');
      await cloudformation
        .createStack({
          StackName: uniqueId,
          TemplateURL: s3Url,
    ...
    if (stackStatus != 'CREATE_COMPLETE' && stackStatus != 'UPDATE_COMPLETE') {
      ...
      console.debug('provisionPreviewStack[pr' + prNumber + '] - Stack Error - ' + stackStatus);
      await octokit.issues.createComment({
        owner,
        repo,
        issue_number: prNumber,
      ...
      console.debug('provisionPreviewStack[pr' + prNumber + '] - Preview Stack succeed');
    ...
    console.log('provisionPreviewStack[pr' + prNumber + '] - Error - ' + err);
  }
}
